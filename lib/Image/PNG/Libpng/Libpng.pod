...
Image::PNG::Libpng::Libpng - Perl interface to the C library "libpng".



=head1 SYNOPSIS

     use Image::PNG::Libpng::Libpng;
     my $png = Image::PNG::Libpng::Libpng::create_read_struct ();
     open my $file, '<:raw', 'nice.png';
     $png->init_io ($file);
     $png->read_png ();
     close $file;

Image::PNG::Libpng::Libpng enables Perl to use the "libpng" library for
reading and writing files in the PNG (Portable Network Graphics)
format.

Image::PNG::Libpng::Libpng consists of Perl subroutines which mirror
the C functions in libpng, plus helper subroutines to make it easier
to read and write PNG data in Perl. 

For an easy way to access PNG files, try the companion module L<Image::PNG::Libpng>, which offers a simplified interface to the functions
in this module.
...
L<in the file C<perl-libpng.c> in the top directory of the distribution|http://cpansearch.perl.org/src/BKB/Image-PNG-Libpng-0.21/perl-libpng.c>.
...
containing transformations to apply to the image:
     ...
     use Image::PNG::Libpng::Const qw/PNG_TRANSFORM_STRIP_ALPHA/;
...
Image::PNG::Libpng::Libpng automatically puts it into Perl's internal
...
=head2 color_type_name
    ...
    my $name = Image::PNG::Libpng::Libpng::color_type_name ($color_type);
...
=head2 text_compression_name
    ...
    my $name = Image::PNG::Libpng::Libpng::text_compression_name ($text->{compression});
...
=head2 get_libpng_ver
    ...
    my $libpng_version = Image::PNG::Libpng::Libpng::get_libpng_ver ();
...
=head2 access_version_number
    ...
    my $libpng_version_number = Image::PNG::Libpng::Libpng::access_version_number ();
...
the module:
    ...
    use Image::PNG::Libpng::Libpng ':all';
    # Now everything in the module has been imported

=head1 Differences from libpng

The procedures in Image::PNG::Libpng::Libpng are closely based on
those of libpng, with the following differences.

=head2 No info structure

This module, C<Image::PNG::Libpng::Libpng> does not use the "info"
...
Image::PNG::Libpng::Libpng, both the "png" and the "info" are contained in
...
you can find each instance L<in the file C<perl-libpng.c> in the top directory of the distribution|http://cpansearch.perl.org/src/BKB/Image-PNG-Libpng-0.21/perl-libpng.c> in the macro
C<UNUSED_ZERO_ARG>.
...
Image::PNG::Libpng::Libpng uses only the return value. Errors and
